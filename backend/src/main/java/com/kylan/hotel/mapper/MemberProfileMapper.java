...
public interface MemberProfileMapper {
    ...
    @Insert("""
            INSERT INTO member_profile(property_id, member_no, member_name, mobile, gender, level_code, point_balance, status,
                                       created_by, updated_by, deleted)
            VALUES(#{propertyId}, #{memberNo}, #{memberName}, #{mobile}, #{gender}, #{levelCode}, #{pointBalance}, #{status},
    ...
    @Select("""
            SELECT m.id, m.property_id AS propertyId, p.property_name AS propertyName,
                   m.member_no AS memberNo, m.member_name AS memberName, m.mobile, m.gender,
    ...
    @Select("""
            SELECT id, property_id AS propertyId, member_no AS memberNo, member_name AS memberName, mobile,
                   gender, level_code AS levelCode, point_balance AS pointBalance, status,
